...
**Last updated:** 2026-06-19 · **Current release:** `v0.22.0` (+ content-intel reliability patch) · **Prod:** [researchtools.net](https://researchtools.net) (Cloudflare Pages + D1)
...
- 🔄 **Auth resilience** — `getUserFromRequest` hardened against guest-hash collisions (8→32-char prefix) and one transient-D1 retry. **Residual:** burst D1 throttling still surfaces as a transient 401 — see Now/#2.
...
2. **Auth resilience — 503 on DB error** `TD-residual` — under burst load, a swallowed D1 error in `getUserFromRequest` returns `null` → spurious **401**. The fix is to signal a **retryable 503** on DB error instead of 401, so clients retry rather than treat it as auth failure.
   - *Files:* `functions/api/_shared/auth.ts` (or wherever `getUserFromRequest` lives) + the callers that map `null` → 401.
   - *Done when:* a transient D1 failure yields 503 (retryable), not 401; normal "no/invalid hash" still yields 401.
   - *Note:* this intermittent 401 also blocked **runtime verification of the Tier-1 consent gate** — confirm the gate fires (403 → consent dialog → success) once this lands.

3. **Security/audit log decision** `TD-03b` — `auth_logs` / `settings_audit_log` record nothing (0 rows). Either populate them on the relevant paths or route them to `event_logs` and drop the dead tables.
   - *Done when:* there's exactly one auth/audit trail that is actually written, and no empty "audit" tables implying coverage that doesn't exist.

4. **SAT correctness & safety fixes** `D0` — a 2026-06-19 audit found **bugs in shipped analytic techniques that change the answers analysts get** (theme #1). Full list + file:line + fixes in [`plans/2026-06-19-analytic-capability-expansion.md`](plans/2026-06-19-analytic-capability-expansion.md) → "Workstream D0." Headline items:
...
- **ACH currently ranks hypotheses by net support, not Heuer disconfirmation** — the flagship technique ships a methodologically inverted ranking until Now/#4 lands; deception AI falls back to a fabricated stub when the (unset) client key is absent. These are live.
...
- **Tier-1 consent gate's runtime fire is unverified** — blocked by the intermittent 401 (Now/#2); verify once that lands.
