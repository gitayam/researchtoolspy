...
2. 🔄 **Auth resilience** `partial (2026-06-15)` — `getUserFromRequest` hardened: guest `username`/`email` now derive from a **32-char** hash prefix (was 8 → two hashes sharing 8 chars collided on the UNIQUE username and permanently 401'd the second; **fixed + verified** — two same-prefix hashes now resolve to distinct guests), and the `SELECT` retries once + re-`SELECT`s after a failed INSERT (insert-race / brief D1 blip). **Residual:** under heavy burst load D1 query-throttling still surfaces as transient 401 (a DB error is swallowed → `null` → 401); the deeper fix is to signal a retryable **503** on DB error instead of 401 (needs caller updates) — low priority since normal traffic won't hit the throttle.
