...
- [ ] **1.4 Backfill `expires_at`** *(GATED — needs cascade-safe guard from 1.2 + backup + approval)* — `UPDATE content_analysis SET expires_at = datetime(created_at,'+7 days') WHERE expires_at IS NULL AND (is_saved = FALSE OR is_saved IS NULL) AND NOT EXISTS (SELECT 1 FROM saved_links sl WHERE sl.analysis_id = content_analysis.id)`. Test on local first.
