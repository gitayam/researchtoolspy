...
Temporal coverage is now visible in the analyst workspace: recorded precision,
intervals, placement-only events and date-unknown events are summarized without
changing saved data. See the [temporal coverage receipt](./2026-09-15-timeline-temporal-coverage-release-receipt.md). TL-06 remains in progress.
...
- **TL-06: delivered.** Calendar diagnostics, precision-preserving date entry, calendar extents, local and durable interval authoring with workspace/draft v2, interval presentations, and immutable v1/v2 history are delivered. Approximate (circa) recorded dates are delivered as workspace v3. Behaviour timelines declare a time domain with parsed relative offsets. Anchor mapping places a relative sequence on host anchors as windows, never instants, surfacing anchor disagreement as a conflict. `timeline-analysis.v2` is defined with its JSON Schema, v1 compatibility layer, evidence-locator semantics and migration guide, and is deliberately not served: v1 remains the only advertised extraction contract until that gate is reviewed.
- **TL-01, TL-05, TL-07–TL-16 and TL-18: pending.** No delivery of these checkpoints or cross-app changes is claimed. TL-07, TL-08, TL-12 and TL-16 list TL-06 among their prerequisites, which is now satisfied.
