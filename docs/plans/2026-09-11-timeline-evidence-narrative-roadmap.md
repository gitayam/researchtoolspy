...
| ResearchTools dedicated tool | `Timeline Analysis`, `Start with what you know`, `Extract an article`, and `Working event sequence` | Immutable extraction plus a temporary Basic/Robust analyst overlay, gaps, questions, findings, AI suggestions, sort, and jump navigation | `timeline-analysis.v1` result plus browser-local investigation state; currently no server-side artifact |
