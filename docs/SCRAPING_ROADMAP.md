...
- **Verification:** scraping-surface and full-project type checks passed; 71 focused Chromium tests passed; 400 broad scraping/content-intelligence tests passed across Chromium and mobile Safari; and the production build passed. Provider behavior was verified with deterministic mocks, without live provider calls.
...
### Orchestrated YouTube route checkpoint — 2026-09-02

- **Delivery class:** integration-enabling safety slice for authenticated users of `/api/content-intelligence/social-extract` and `/api/content-intelligence/social-media-extract`. Existing response shapes remain route-specific; direct Cobalt media is deliberately replaced by the canonical Watch-on-YouTube fallback.
- **Baseline and topology:** canonical `origin/main` was freshly verified and published at `359fac864e5bb1d4137ce35b1d5375e484f2dd28`. The configured GitLab mirror was read-only and 61 commits behind. A sequential shared-provider then two-route topology used isolated worktrees; inventory, roadmap, join, release gates, and canonical push remain integration-owner responsibilities.
- **Provider provenance:** the bounded shared provider candidate `7e2e697ee34bf6e79bdc5d51ccf58714a1af23c2` and correction `40934db63d86093cf7e4a27d9b2f008229fe8772` joined as `12b6dc863`/`77c4f6791`. The route candidate `6de760803513e918ceb32a8b006906f2a2988c4b` and correction `29cf235764faaf50b5a48a119cf7f58d61647547` joined as `bb670e1e5`/`038dbf89f`.
- **Independent review:** the provider join was held until caller abort became terminal, oEmbed output URLs used exact approved origins, and Cobalt transport/output was removed. The route join was held until YouTube became structurally ephemeral, legacy non-YouTube platform identity was restored, and malformed exact-authority YouTube URLs retained their established client envelope.
...
- **Verification:** 135 focused Chromium parser/route/inventory tests passed; 736 broad scraping/content-intelligence tests passed across Chromium and mobile Safari; full-project and scraping-surface type checks passed; focused lint/diff checks passed; and the production build passed. All provider behavior used deterministic mocks with no live provider calls.
- **Behavioral invariant:** after authentication, valid and invalid Instagram requests perform zero provider, DNS, extraction-cache KV, and extraction-persistence D1 work; canonical identity never retains query data; pre-aborted requests are terminal; other social platforms remain on their existing constrained paths.
- **Remaining constrained scope:** TikTok, Facebook, and Bluesky branches retain legacy parsing/provider behavior, so INV-019 and INV-020 remain `constrained-provider`. Shared DNS validation still requires an enforcing connect-time egress boundary.

### Twitter/X route checkpoint — 2026-09-02

- **Delivery class:** first-party provider migration for both authenticated content-intelligence social routes. Standard `x.com` and `twitter.com` post links canonicalize to one `x.com` identity while route-specific response shapes remain intact.
- **Falsifiable hypothesis:** one exact canonical target plus one bounded first-party X oEmbed request can preserve useful post text and author identity while invalid, spoofed, or aborted requests perform zero provider, cache, and persistence work. Deterministic route/provider fixtures passed; the hypothesis remains subject to production provider-health telemetry.
- **Provider boundary:** requests use only `https://publish.x.com/oembed`, canonical target input, public-DNS checks, zero redirects, a 10-second timeout, JSON-only MIME, and a 128 KiB response ceiling. Provider HTML is reduced to at most 10,000 characters of text and is never returned.
- **Retired behavior:** both route paths no longer use `publish.twitter.com`, Cobalt, VxTwitter, `pic.twitter.com` extraction, arbitrary provider-returned media URLs, raw embed HTML, or Nitter guidance. Media modes return only a canonical open-on-X action.
