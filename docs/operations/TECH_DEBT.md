...
### TD-04 · Composite coverage + redundant-index write amplification — 🔄 NARROW CLEANUP DEPLOYED, TELEMETRY PENDING `MED`
...
- **Narrow cleanup deployed (2026-07-28):** five-day post-0004 insights showed 81 `content_analysis` inserts at 13 average rows written and 2.828 ms average. Managed migration `0005_drop_redundant_content_analysis_indexes.sql` removed `_hash` and `_user` through a fresh backed-up release; their left-prefix workspace composites now serve the single-column plans. Production has 551 indexes and 10 on `content_analysis`.
- **Debt that remains:** quantify the write reduction from fresh post-0005 inserts. Review other single workspace/session indexes separately; retain type-only indexes because `(workspace_id, type, ...)` cannot serve a type-only predicate.
- **Done when:** fresh insights confirm the content insert write reduction and any further removal is supported by source plus planner evidence without query regressions.
