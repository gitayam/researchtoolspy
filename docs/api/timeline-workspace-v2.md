...
TimelineJS presentations include recorded interval events with both endpoint labels. Its optional `end_date` preserves the recorded calendar components and clock precision; no missing day, month, time or timezone is added to the exported fields. TimelineJS lays out partial dates using default components internally, so the displayed span is a presentation convention, not an exact duration. If those defaults would put an overlapping-precision end before its start, the slide retains the full recorded range label and explains why no span is drawn. The complete ResearchTools JSON backup preserves both endpoints in every case. See [TimelineJS date fields](https://timeline.knightlab.com/docs/json-format.html).

Presentation scheduling keeps recorded intervals fixed. The schedule displays their recorded range without date/time override controls, and the adapter rejects interval date/time or nondefault meaning overrides. Automatic steps can follow a complete, explicitly timed recorded end; an untimed or partial end does not provide a clock anchor, and a warning explains use of the previous usable anchor or default. Versioned private interval snapshots remain a subsequent stage.
