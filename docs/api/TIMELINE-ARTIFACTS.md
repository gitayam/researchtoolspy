...
The snapshot kinds `timeline-workspace.v1` and `timeline-workspace.v2` each carry one complete `TimelineWorkspaceExport` as its payload: `schemaVersion`, `exportedAt`, `source`, and `analystWorkspace`. The shared strict local-import codec validates it, including original extraction, original event wording, stable event IDs, analyst edits, narrative chapters and roles, questions, hypotheses and their references. Unknown fields or invalid references fail closed. Storage retains the submitted payload shape; it does not truncate or convert the workspace to an event candidate. The submitted kind must equal the payload’s `schemaVersion`. Version 2 retains recorded interval endpoints and their precision; version 1 rejects interval fields. Existing `event-candidate.v1` validation and write hashes remain compatible.
...
The browser uses the stable object ID `browser-workspace` and the snapshot’s exact `timeline-workspace.v1` or `timeline-workspace.v2` kind. Adding or removing an interval preserves the artifact and object identity; old revisions keep their original format. Each explicit save commits the complete workspace as one immutable snapshot in one PATCH. A new save first creates an empty artifact and then commits its snapshot; an interrupted initial save may leave an empty artifact, and retry reuses its original create/commit keys. The browser checks the full snapshot size before either request, leaving oversized timelines open for local editing and JSON export.
...
Version hashes use SHA-256 of canonical JSON `{schemaVersion:kind,tombstone,payload}`, where kind is `event-candidate.v1`, `timeline-workspace.v1` or `timeline-workspace.v2`. Existing candidate hash inputs are unchanged. Canonical JSON sorts object keys and preserves array order. Revision hashes cover the binary-object-ID-sorted manifest entries, including stable version IDs, kind, tombstone and content hash. Historical detail recomputes manifest hashes. Object reads validate payload schemas and recompute version hashes; malformed stored snapshots return `503 datastore_unavailable`. Payload versions are never replaced by current versions when reading history. Commit and revision-manifest queries fetch only identity/hash metadata, avoiding loading unchanged snapshot bodies; full payloads are fetched only through bounded object pages.

## Migration 0015: interval workspace versions

`0015_timeline_workspace_intervals.sql` extends the two object-kind constraints and version-kind guard to the explicit workspace v1/v2 family. The immutable object creation kind is retained; each new version records its own schema. Candidate/workspace transitions and unknown versions remain prohibited. All old payloads, hashes, versions, manifests and retry records remain unchanged.

Apply the entire migration and its tracking entry atomically, with deferred foreign keys, using the same reconstruction boundary as 0012 below. Rehearse the actual production prefix and verify existing rows and dependent catalog before application. Rolling back application code leaves interval history intact, but an older application may refuse to open v2 revisions. Restore compatible application code rather than narrowing constraints or deleting history.
