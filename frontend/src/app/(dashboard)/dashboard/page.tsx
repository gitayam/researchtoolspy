    ...
    <div className="space-y-6">
          ...
          <CardContent>
            {recentSessions.length > 0 ? (
                ...
                {recentSessions.slice(0, 5).map((session) => (
                  <div key={session.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      session.status === 'in_progress' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' :
              ...
              <div className="text-center py-6">
                <Brain className="h-12 w-12 text-gray-400 dark:text-gray-500 mx-auto mb-2" />
                <p className="text-sm text-gray-500 dark:text-gray-400">No recent activity</p>
                <p className="text-xs text-gray-400 dark:text-gray-500">Start a new analysis to see it here</p>
              </div>
            )}
          </CardContent>
        </Card>
                ...
                <div key={framework.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{framework.name}</span>
                      <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{framework.count}</span>
                    </div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{framework.trend}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card className="border-dashed border-2 border-gray-300 dark:border-gray-600">
        <CardContent className="flex flex-col items-center justify-center py-8">
          ...
          <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-gray-100">Start a New Analysis</h3>
