-- Add missing public sharing fields to ACH analyses
-- Migration 052: Add remaining sharing columns that don't exist yet

-- Check and add missing columns (is_public already exists)
-- ALTER TABLE ach_analyses ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0; -- SKIP: already exists

-- Add share_token if not exists
...
-- Add clone_count if not exists
ALTER TABLE ach_analyses ADD COLUMN clone_count INTEGER NOT NULL DEFAULT 0;

-- Add domain if not exists
ALTER TABLE ach_analyses ADD COLUMN domain TEXT;

-- Add tags if not exists
ALTER TABLE ach_analyses ADD COLUMN tags TEXT;

-- Add shared_publicly_at if not exists
...
-- Collaboration table for team analysis
