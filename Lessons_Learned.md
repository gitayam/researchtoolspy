...
## Authentication State Persistence in Next.js/React

### ❌ What Didn't Work

**Problem**: Authentication state lost when navigating between pages
```typescript
// ❌ Hardcoding authentication state in components
const isAuthenticated = false  // Always false, ignores actual auth!

// ❌ Direct use of hooks without initialization
const isAuthenticated = useIsAuthenticated()  // False during SSR/initial render
```

**Root Causes**:
1. **SSR/Client Hydration Mismatch**: Server doesn't have access to localStorage, causing auth to be false on initial render
2. **Hardcoded Values**: Components with `const isAuthenticated = false` override actual auth state
3. **Race Conditions**: API client and auth store initializing at different times
4. **Storage Key Mismatches**: Using 'omnicore-auth' vs 'omnicore_auth' (hyphen vs underscore)

**Symptoms**:
- User logs in successfully but appears logged out on navigation
- Register page repeatedly generating new hashes for authenticated users
- Framework pages showing logged-out state despite valid tokens

### ✅ What Worked

**Solution 1**: Create AuthGuard component for proper initialization
```typescript
...
export function AuthGuard({ children, fallback = null }: AuthGuardProps) {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      // If tokens exist but store not authenticated, refresh
      if (storedTokens && !isAuthenticated) {
        await refreshUser()
      }
      
      // If store says authenticated but no tokens, clear auth
...
**Solution 2**: Wrap all auth-dependent layouts
```typescript
// ✅ In layout files
...
**Solution 3**: Fix API client initialization with retry logic
```typescript
// ✅ In auth store refreshUser
let retries = 0
while (!apiClient && retries < 5 && typeof window !== 'undefined') {
  await new Promise(resolve => setTimeout(resolve, 100))
  retries++
}
```

**Solution 4**: Use Proxy pattern for lazy API client initialization
```typescript
// ✅ API client singleton with SSR safety
export const apiClient = new Proxy({} as APIClient, {
  get(target, prop) {
    if (typeof window !== 'undefined' && !apiClientInstance) {
...
1. **Never hardcode auth state** - Always use hooks from auth store
2. **Wrap with AuthGuard** - All pages checking authentication need AuthGuard
3. **Handle SSR properly** - Check `typeof window !== 'undefined'` before accessing localStorage
4. **Use consistent storage keys** - Standardize on 'omnicore_tokens' and 'omnicore_auth'
5. **Add auth redirects** - Login/register pages should redirect if already authenticated
6. **Test navigation flows** - Verify auth persists across all page transitions

### Key Learnings

1. **SSR is tricky** - Next.js renders on server first, so localStorage isn't available
2. **Initialization order matters** - Auth must be restored before components check it
3. **Synchronization is critical** - API client tokens and auth store must stay in sync
4. **Fallback states are important** - Show loading or nothing while auth initializes
5. **Debug with extensive logging** - Add console.logs to track auth flow during development

### Testing Checklist

- [ ] Login persists on page refresh
- [ ] Navigation to framework pages maintains auth
- [ ] Register page redirects if already authenticated
- [ ] Login page redirects if already authenticated
- [ ] Logout clears all auth state and tokens
- [ ] Browser back/forward maintains correct auth state
...
- Create AuthGuard component to handle initialization
- Never hardcode authentication values
- Handle SSR/client hydration properly
- Synchronize localStorage with state management
- Add retry logic for async initialization
