...
test.describe('durable browser with actual D1 routes @smoke', () => {
  ...
  test('revision history inspects old snapshots without replacing edits or save head', async ({ page }, info) => {
    ...
    try {
      const value = fixture()
      Object.assign(value.analystWorkspace.events[0], { placement: { mode: 'absolute' }, eventDate: '2024-01-02', datePrecision: 'day', narrativeIncluded: true, narrativeOrder: 0 })
      await page.goto('/dashboard/tools/timeline'); await importFixture(page, value)
      await page.getByRole('button', { name: 'Save timeline', exact: true }).click(); await saved(page)
      ...
      const presentation = JSON.parse(await readFile((await (await presentationDownload).path())!, 'utf8'))
      expect(presentation.events).toHaveLength(1)
      expect(presentation.events[0].start_date).toMatchObject({ year: 2024, month: 1, day: 2 })
      expect(presentation.title.text.headline).toBe(value.analystWorkspace.narrative!.title)
      ...
      await selected.getByRole('button', { name: 'Preview selected revision', exact: true }).click({ trial: true })
      await selected.screenshot({ path: info.outputPath('history-card.png'), scale: 'css', animations: 'disabled' })
      await page.screenshot({ path: info.outputPath('history-viewport.png'), scale: 'css', animations: 'disabled' })
      await history.getByRole('button', { name: 'Refresh history', exact: true }).evaluate(element => element.scrollIntoView({ block: 'center' }))
      await history.getByRole('button', { name: 'Refresh history', exact: true }).click({ trial: true })
      await page.screenshot({ path: info.outputPath('history-list-viewport.png'), scale: 'css', animations: 'disabled' })
