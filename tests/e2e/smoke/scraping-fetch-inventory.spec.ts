...
const entries: InventoryEntry[] = [
  ...
  { id: 'INV-005', file: 'functions/api/tools/extract-claims.ts', status: 'safe-multi-source', evidence: /fetchArchivePhSource\(url,/, forbidden: [/enhancedFetch\(/, /await fetch\(/, /cdn\.ampproject\.org/, /webcache\.googleusercontent\.com/], typecheck: 'root' },
  { id: 'INV-006', file: 'functions/api/tools/extract-timeline.ts', status: 'safe-multi-source', evidence: /scrapeUrl\(url, undefined, \{ purpose: 'timeline', allowArchives: true \}\)/, forbidden: [/enhancedFetch\(/, /renderArticleFallback/, /BROWSER_RENDERER/], typecheck: 'root' },
  { id: 'INV-007', file: 'functions/api/ai/scrape-url.ts', status: 'safe-multi-source', evidence: /scrapeUrl\(normalizedUrl, context\.env\.APIFY_API_KEY, \{\s*purpose: 'framework',\s*allowArchives: false,\s*\}\)/, forbidden: [/fetch\(url[,)]/, /function extractTextFromHTML/, /allowArchives: true/], typecheck: 'root' },
  ...
  { id: 'INV-011', file: 'functions/api/tools/rage-check.ts', status: 'safe-multi-source', evidence: /purpose: 'rage-check',[\s\S]*allowArchives: true/, forbidden: [/BROWSER_RENDERER/, /renderArticleFallback/], typecheck: 'root' },
  ...
  { id: 'INV-020', file: 'functions/api/content-intelligence/social-media-extract.ts', status: 'safe-provider', evidence: /parseCanonicalBlueskyUrl\(url\)/, forbidden: [/function detectPlatform/, /function extractYouTubeVideoId/, /function fetchYouTubeTranscript/, /function extractInstagramVia/, /fetchWithRetry/, /getCached/, /saveSocialMediaExtraction/, /social_media_extractions/, /com\.atproto\.identity\.resolveHandle/, /post\.embed\.playlist/, /img\.fullsize/, /snapinsta\.app/, /instadp\.com/, /saveinsta\.app/, /api\.instagram\.com\/oembed/, /publish\.twitter\.com/, /api\.vxtwitter\.com/, /co\.wuk\.sh/], typecheck: 'root' },
  {
    id: 'INV-021',
