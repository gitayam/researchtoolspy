...
test.describe('Cloudflare account selection @smoke', () => {
  ...
  test('@smoke deployment verifies required secrets and analysis metering', () => {
    ...
    const runbook = read('docs/operations/SCRAPING_DEPLOYMENT.md')
    expect(deploy).toContain('REQUIRED_SECRETS="TRUSTED_ANALYSIS_KEYS OPENAI_API_KEY JWT_SECRET SCRAPE_TELEMETRY_KEY PRODUCT_TELEMETRY_KEY INTEGRATION_TOKEN_HASH_KEY"')
