...
export function decodeTimelineWorkspace(text: string): TimelineWorkspaceExport {
  ...
  const workspace = object(root.analystWorkspace, 'analystWorkspace', ['mode', 'events', 'questions', 'hypotheses', 'narrative', 'presentation', 'sortDirection', 'evidence', 'analysis'])
  ...
  if(workspace.analysis!==undefined) validateTimelineJudgments(workspace.analysis,result.analystWorkspace.events,result.analystWorkspace.evidence)
