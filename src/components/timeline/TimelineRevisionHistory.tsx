...
export function TimelineRevisionHistory({ artifactId, identity }: { artifactId: string; identity: () => DurableIdentity }) {
  ...
  return <section aria-label="Saved revision history" className="min-w-0 space-y-3 rounded-lg border border-violet-200 bg-violet-50/50 p-3 dark:border-violet-900 dark:bg-violet-950/20">
    ...
    {page && <>
      ...
      <ol className="max-h-80 space-y-2 overflow-y-auto rounded-md" aria-label="Saved revisions" tabIndex={0}>
        {revisions.map(revision => <li key={revision.revisionId} className="min-w-0 rounded-md border bg-background p-3">
          ...
          <details className="mt-1 text-xs text-muted-foreground"><summary className="cursor-pointer font-medium">Revision details</summary><p className="mt-1 break-all">{revision.revisionId}</p></details>
    ...
    {selected && <section aria-label="Selected historical revision" className="space-y-2 rounded-md border border-indigo-300 bg-background p-3 dark:border-indigo-800">
      ...
      <details className="text-xs text-muted-foreground"><summary className="cursor-pointer font-medium">Revision details</summary><p className="mt-1 break-all">Revision ID: {selected.revision.revisionId}</p></details>
