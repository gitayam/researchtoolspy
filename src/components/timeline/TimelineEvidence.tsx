    ...
    <div className="mt-3 space-y-4 break-words text-sm">
      ...
      <details className="rounded border p-3"><summary className="cursor-pointer font-medium">Add or edit source</summary>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <label className="sm:col-span-2">Source to edit<select className={selectStyle} value={editingSource ?? ''} onChange={change => { const item=data.sources.find(s=>s.id===change.target.value); setEditingSource(item?.id ?? null); setSource(item ? {url:item.url,title:item.title,publisher:item.publisher,publishedAt:item.publishedAt??'',retrievedAt:item.retrievedAt??''}:blankSource) }}><option value="">New source</option>{data.sources.map(item=><option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
      ...
      <details className="rounded border p-3" open={editingAssertion ? true : undefined}><summary className="cursor-pointer font-medium">Add or edit assertion</summary>
        <div className="mt-3 space-y-3">
          <label className="block">Assertion source<select className={selectStyle} value={sourceId} onChange={change=>setSourceId(change.target.value)}><option value="">Choose source</option>{data.sources.map(item=><option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
          {(['claimText','temporalClaim','quote','locator','observedAt','reportedAt'] as const).map(field=><label key={field} className="block">{({claimText:'Source claim',temporalClaim:'Temporal wording',quote:'Quoted passage',locator:'Passage locator',observedAt:'Observed at (ISO)',reportedAt:'Reported at (ISO)'})[field]}<Textarea value={assertion[field]} maxLength={field==='claimText'||field==='quote'?4000:1000} onChange={change=>setAssertion({...assertion,[field]:change.target.value})}/></label>)}
          <label className="block">Derives from assertions<select multiple className={selectStyle} value={assertion.derivesFrom} onChange={change=>setAssertion({...assertion,derivesFrom:Array.from(change.target.selectedOptions,option=>option.value)})}>{data.assertions.filter(item=>item.id!==editingAssertion).map(item=><option key={item.id} value={item.id}>{item.claimText}</option>)}</select></label>
          {!editingAssertion && <label className="block">New assertion relation<select className={selectStyle} value={relation} onChange={change=>setRelation(change.target.value as TimelineEvidenceLink['relation'])}>{['supports','contradicts','context'].map(value=><option key={value}>{value}</option>)}</select></label>}
      ...
      <fieldset className="space-y-2 rounded border p-3"><legend>Link an existing assertion</legend>
        <label className="block">Existing assertion<select className={selectStyle} value={existingAssertion} onChange={change=>setExistingAssertion(change.target.value)}><option value="">Choose assertion</option>{data.assertions.filter(item=>!data.links.some(link=>link.eventId===event.id&&link.assertionId===item.id)).map(item=><option key={item.id} value={item.id}>{item.claimText}</option>)}</select></label>
        <label className="block">Link relation<select className={selectStyle} value={relation} onChange={change=>setRelation(change.target.value as TimelineEvidenceLink['relation'])}>{['supports','contradicts','context'].map(value=><option key={value}>{value}</option>)}</select></label>
      ...
      <fieldset className="space-y-3 rounded border p-3"><legend>Review corroboration</legend>
        ...
        <label className="block">Independence<select className={selectStyle} value={independence} onChange={change=>setIndependence(change.target.value as TimelineEvidenceReview['independence'])}>{['unresolved','independent','dependent'].map(value=><option key={value}>{value}</option>)}</select></label>
        <label className="block">Compatibility<select className={selectStyle} value={compatibility} onChange={change=>setCompatibility(change.target.value as TimelineEvidenceReview['compatibility'])}>{['unresolved','compatible','incompatible'].map(value=><option key={value}>{value}</option>)}</select></label>
