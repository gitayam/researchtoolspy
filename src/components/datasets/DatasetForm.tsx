...
interface DatasetFormProps {
  open: boolean
  onClose: () => void
  onSave: (data: DatasetData) => Promise<void>
...
export function DatasetForm({ open, onClose, onSave, initialData, mode }: DatasetFormProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{mode === 'create' ? 'Add New Dataset' : 'Edit Dataset'}</DialogTitle>
          <DialogDescription>
            {mode === 'create'
              ? 'Add a new piece of dataset to your collection'
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving}>
              {saving ? 'Saving...' : mode === 'create' ? 'Add Dataset' : 'Save Changes'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
