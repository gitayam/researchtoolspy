-- Workspace-family v1/v2 versions share immutable creation identities.
-- All historical bytes remain unchanged; only explicit family transitions widen.
...
-- statement
CREATE TABLE timeline_objects_0015_backup AS SELECT * FROM timeline_objects;
-- statement
CREATE TABLE timeline_versions_0015_backup AS SELECT * FROM timeline_object_versions;
-- statement
...
CREATE TABLE timeline_objects (
  ...
  id TEXT NOT NULL,
  kind TEXT NOT NULL CHECK(kind IN ('event-candidate.v1','timeline-workspace.v1','timeline-workspace.v2')),
...
CREATE TABLE timeline_object_versions (
  ...
  id TEXT NOT NULL,
  schema_version TEXT NOT NULL CHECK(schema_version IN ('event-candidate.v1','timeline-workspace.v1','timeline-workspace.v2')),
...
-- statement
INSERT INTO timeline_objects SELECT * FROM timeline_objects_0015_backup;
-- statement
INSERT INTO timeline_object_versions SELECT * FROM timeline_versions_0015_backup;
-- statement
DROP TABLE timeline_versions_0015_backup;
-- statement
DROP TABLE timeline_objects_0015_backup;
-- statement
...
CREATE TRIGGER timeline_version_kind BEFORE INSERT ON timeline_object_versions BEGIN
  SELECT (CASE WHEN NOT EXISTS (
    SELECT 1 FROM timeline_objects o WHERE o.workspace_id=NEW.workspace_id AND o.artifact_id=NEW.artifact_id
      AND o.id=NEW.object_id AND (o.kind=NEW.schema_version OR (o.kind IN ('timeline-workspace.v1','timeline-workspace.v2') AND NEW.schema_version IN ('timeline-workspace.v1','timeline-workspace.v2')))
