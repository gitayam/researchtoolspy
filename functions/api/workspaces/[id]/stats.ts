...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // COP sessions (match by team_workspace_id OR workspace_id), frameworks, members
    const [copSessions, frameworks, members] = await Promise.all([
      env.DB.prepare('SELECT COUNT(*) as c FROM cop_sessions WHERE team_workspace_id = ? OR workspace_id = ?').bind(workspaceId, workspaceId).first(),
    ...
    const toolsResult = await env.DB.prepare(`
      SELECT
        (SELECT COUNT(*) FROM cop_playbooks WHERE cop_session_id IN (SELECT id FROM cop_sessions WHERE team_workspace_id = ? OR workspace_id = ?)) +
        (SELECT COUNT(DISTINCT t.id) FROM cop_task_templates t JOIN cop_sessions cs ON cs.workspace_id = t.workspace_id WHERE cs.team_workspace_id = ? OR cs.workspace_id = ?) +
        (SELECT COUNT(*) FROM cop_intake_forms WHERE cop_session_id IN (SELECT id FROM cop_sessions WHERE team_workspace_id = ? OR workspace_id = ?))
        as total
    `).bind(workspaceId, workspaceId, workspaceId, workspaceId, workspaceId, workspaceId).first()
