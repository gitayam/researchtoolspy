/**
 * COP Public Intake — Slug Resolver
 *
 * GET /api/cop/public/intake/by-slug/:slug
 * Resolves a custom slug to the form's share token, then returns the form schema.
 */
  ...
  try {
    const form = await env.DB.prepare(
      `SELECT id, title, description, form_schema, require_location, require_contact,
       ...
       FROM cop_intake_forms WHERE custom_slug = ?`
    ...
    console.error('[COP Public Intake] Slug resolve error:', error)
