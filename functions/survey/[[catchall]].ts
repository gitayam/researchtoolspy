 ...
 *
 * Intercepts /survey/* requests. For bot user-agents, fetches survey data
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const pathParts = url.pathname.replace('/survey/', '').split('/').filter(Boolean)
  ...
  const canonicalUrl = survey.custom_slug
    ? `${siteUrl}/survey/${survey.custom_slug}`
    : `${siteUrl}/survey/${survey.share_token}`
