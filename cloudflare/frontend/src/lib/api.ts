...
export class APIClient {
  ...
  constructor() {
    console.log('🏗️ API Client: Constructor called, initializing...')
    ...
    console.log('🏗️ API Client: Loading tokens on initialization...')
  ...
  private loadTokensFromStorage(): void {
    try {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        ...
        console.log('🔍 API Client: Loading tokens from storage:', stored ? 'Found tokens' : 'No tokens found')
        if (stored) {
          this.tokens = JSON.parse(stored)
          console.log('✅ API Client: Tokens loaded successfully, expires at:', new Date(Date.now() + (this.tokens?.expires_in || 0) * 1000).toLocaleTimeString())
        }
      }
    } catch (error) {
      console.error('❌ API Client: Failed to load tokens from storage:', error)
    }
  }
  ...
  private saveTokensToStorage(tokens: AuthTokens): void {
    try {
      // Check if we're in the browser environment
      if (typeof window !== 'undefined' && window.localStorage) {
        console.log('💾 API Client: Saving tokens to storage, expires in:', tokens.expires_in, 'seconds')
        localStorage.setItem('omnicore_tokens', JSON.stringify(tokens))
        console.log('✅ API Client: Tokens saved successfully')
      }
      this.tokens = tokens
    } catch (error) {
      console.error('❌ API Client: Failed to save tokens to storage:', error)
    }
  }

  private clearTokens(): void {
    console.log('🗑️ API Client: Clearing tokens from storage')
    this.tokens = null
    if (typeof window !== 'undefined') {
  ...
  isAuthenticated(): boolean {
    ...
    console.log('🔐 API Client: isAuthenticated check:', hasTokens, this.tokens?.access_token ? 'Token exists' : 'No token')
    return hasTokens
  }
...
export const apiClient = new Proxy({} as APIClient, {
  get(target, prop) {
    ...
    if (typeof window !== 'undefined' && !apiClientInstance) {
      console.log('🎯 API Client: Initializing singleton on first access')
